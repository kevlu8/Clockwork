...
void History::update_correction_history(const Position& pos, i32 depth, i32 diff) {
    ...
    u64   material_key       = pos.get_material_key();
    ...
    usize material_index = static_cast<usize>(material_key % CORRECTION_HISTORY_ENTRY_NB);
    ...
    update_entry(m_material_corr_hist[side_index][material_index]);
}
...
i32 History::get_correction(const Position& pos) {
    ...
    correction += m_material_corr_hist[side_index][material_index];
